package com.example.project_1;

public class Const {

    public static final String PLAYER_NAME_KEY = "PLAYER_NAME";
    public static final String PLAYER_WINNER_KEY = "WINNER_PLAYER";
    public static final String PLAYER_DATE_KEY = "DATE_PLAYER";
    public static final String PLAYER_RECORD_KEY = "RECORD_PLAYER";
    public static final String RECORDS_LIST_KEY = "RECORDS_LIST";
    public static final int RECORDS_TOP_TEN = 10;
    public static final String PLAYER_SCORE_KEY = "PLAYER_WINNER_SCORE";
}
